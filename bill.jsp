...
try
{
	int total=0;
	int sno=0;
	Connecction con=ConnectinProvider.getCon();
	Statement rs=st.executeQuery("select sum(total)from cart wher email='"+email+"' and status='bill'");
	while(rs.next())
	{
		total=rs.getint(1);
	}
...
<h3>Product Details</h3>
  ...
  <% 
  ResultSet rs1=st.executQurey("select * from cart inner jion product where cart.product_id=product.id and cart.email='"+email+"' and cart.status='bill'");
  while(rs1.next())
  {
	  sno=sno+1;
  %>
