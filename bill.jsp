...
try{
	...
	ResultSet rs2=st.executeQuery("select * from users inner join where cart.email='"+email+"' and cart.status='bill'");
	while(rs2.next()){
...
<div class="right-div-right"><h3>Email:  <%=rs.getString(email) %>  </h3></div>
...
<h3>Product Details</h3>
  ...
  <%
  ResultSet rs1=st.executeQuery("select * from inner join product where cart.product_id=product.id nd cart.email='"+email+"' and cart.status='bill'");
while(rs1.next()){
	sno=sno+1;
}
